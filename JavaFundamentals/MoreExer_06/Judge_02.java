        ...
        while (!input.equals("no more time")) {
            ...
            if (!usersMap.containsKey(contest)) {   //проверка дали курса не съществува
                ...
                for (Map.Entry<String, List<User>> entry : usersMap.entrySet()) {
                    ...
                    for (User user1 : entry.getValue()) {
                        if (user1.getUserName().equals(name) && entry.getKey().equals(contest)) {
                            isExistUser = true;
                            break;
                        }
                    }
...
//                }
    ...
    {                                    //принтиране на основния дневник
    ...
    Map<String, Integer> usersAllPointsMap = new LinkedHashMap<>();
        ...
        for(
    Map.Entry<String, List<User>> entry :usersMap.entrySet())
